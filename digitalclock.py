...
for i in range(table.shape[0]):
	for j in range(table.shape[1]):
		table[i,j]=[0,0,0]
for i in sides:
	cv2.fillPoly(table,pts=[i],color=[8,8,8])#color=(15,15,15)
...
def show(n):
	...
	for j,k in enumerate(n):
		for i in numbers[str(k)]:
			img=cv2.fillPoly(img,pts=[enumerations[str(j+1)][i]],color=color)
	cv2.imshow("Press Esc to ... escape",img)
	k=cv2.waitKey(1)
	sleep(0.99)
	if k==27:
		sys.exit()
while True:
	...
	if answer=="1":
		while True:
			...
			cv2.setMouseCallback("Press Esc to ... escape",clr)
		break
	elif answer=="2":
